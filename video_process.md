...
2. **양자화(Quantization)**: 각 위치의 연속 밝기를 유한한 정수 레벨로 변환
3. **부호화(Encoding)**: 양자화된 값을 이진수로 변환

표본화는 위치를 나누고, 양자화는 값을 나누고, 부호화는 저장
 
> 시험 포인트: "아날로그 신호를 디지털로 바꾸는 과정에서 공간 좌표를 이산 신호로 바꾸는 과정은?
...
F(4,5) = 60       F(4,6) = 100
F(5,5) = 80       F(5,6) = 140
양선형 보간법을 이용하여 F̂(4.4, 5.6)의 값을 구하시오. 풀이 과정을 모두 쓰시오.
...
- **양차 보간(Bicubic)**: 주변 16개 픽셀 사용. 가장 품질 좋지만 계산 비용 높음.
...
1. 입력 영상의 CDF(s_k)를 구한다.
2. 목표 영상의 CDF(v_k)를 구한다.
3. 각 s_k에 대해, v_k ≥ s_k 를 만족하는 가장 작은 z를 찾는다.
...
컴퓨터의 rand() 함수는 균일분포 난수만 생성할 수 있어서, 가우시안 분포를 직접 만들 수 없다. 이를 해결하기 위해 중심극한정리를 이용한다.

중심극한정리란, 독립적인 균일분포 실용적으로 12개를 합산하면 가우시안 분포와 근사가 된다

### 가우시안 분포 PDF
 
>> p(x) = (1 / √(2πσ²)) × exp(-(x-m)² / 2σ²)

우리가 만들고 싶은 잡음의 분포가 이렇게 생겼다
